...
class GaussianRenderer:
    ...
    def __init__(self, image_height: int = 512, image_width: int = 512,
                 fovy: float = 30.00000115370317, zfar: float = 2.5, znear: float = 0.5):
    ...
    def render(self, gaussians, cam_view, cam_view_proj, cam_pos, bg_color=None, scale_modifier=1):
        ...
        images = torch.stack(images, dim=0).view(B, V, 3, self.image_height, self.image_width)
        alphas = torch.stack(alphas, dim=0).view(B, V, 1, self.image_height, self.image_width)
